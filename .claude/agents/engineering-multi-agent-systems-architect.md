...
## Skills
- **dsh-plugin-dev** — invoke via the Skill tool before designing or reviewing any `af-*` runner package, tool, or hook plugin in this repo. It covers the Cordis plugin shape, event dispatch modes, extension-point selection, and the AgentFleet-specific constraints (D14 bundle ownership, `runners` network isolation, in-process-subagent limits) that any multi-agent coordination design here must respect.
